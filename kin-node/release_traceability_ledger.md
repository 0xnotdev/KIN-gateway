...
is named. Evidence links are repository-relative. All automated implementation
requirements pass; remaining `OPEN` labels identify external human or publication
authority that must not be self-attested by the codebase.

## §3.2 measurable acceptance bar

| Requirement | Implementation seam | Automated test ID | Manual step | Result | Evidence |
|---|---|---|---|---|---|
| One-command Windows install under five minutes | pinned checksum-verifying `install.ps1`; isolated environment | `test_release_readiness_m8.py`; clean-wheel install | time clean Windows laptop install | PASS automated; human timing OPEN | `docs/v1.1/INSTALL-WINDOWS.md` |
| Core journey without DB/protocol hand edits | TUI + V1.1 CLI contracts | T8 Phase A/B smoke; `test_cli_v11_contract.py` | two-laptop full journey | PASS automated; human journey OPEN | `tui_t8_phase_a_progress.md` |
...
| Direct session under 5s; offline queue resumes | transport, encrypted durable queue, production 5s sync loop | T8 Phase B/C smoke; runtime scheduler/queue tests | normal two-laptop timing | PASS automated; human timing OPEN | `tests/test_smoke_two_node.py` |
...
| Embedded, webhook, local-command, SDK adapter contract | `adapters/` normalized request/response and capability declarations | invocation/timeout/security contract tests | none | PASS | `tests/test_adapters_contract.py` |
...
| Tag-in creates bounded package and signed participant change; no private memory | orchestrator + real transport + owner-gated AgentPicker UX | bounded/private-exclusion handoff and transport tests | two-laptop visibility | PASS automated; human visibility OPEN | `kin/session/orchestrator.py` |
...
| Dispatch lifecycle and explicit receiver agent confirmation | reducer/transition matrix/TUI Inbox | dispatch/transport/two-node smoke | receiver review | PASS automated; human review OPEN | `kin/session/transition_matrix.py` |
...
| Explicit agent selection and visible signed replacement | transport/orchestrator/Arena tag-in picker and exact confirmation | participant-change, picker and consequential-gate tests | peer visibility on two laptops | PASS automated; human visibility OPEN | `kin/tui/widgets/session_arena.py` |
...
| Trust/provenance is visible and card changes reviewable | Arena trust strip, Inspector, card diff | TUI trust/card tests and four-breakpoint references | 15-second Arena assessment | PASS automated; human assessment OPEN | `kin/tui/widgets/trust_strip.py` |
...
| Sensitive stored content encrypted at rest; public metadata safe | vault + persistence; authenticated legacy-aware reads | raw-database vault, transport-objective, approval-request and projection tests | none | PASS; pre-V1 records remain readable without rewriting history | `kin/storage/vault.py` |
...
| Requirement | Implementation seam | Automated test ID | Manual step | Result | Evidence |
|---|---|---|---|---|---|
| Readable pinned/checksummed one-command PowerShell installer with pipx alternative | `install.ps1`; same-release wheel/manifest verification | release readiness + clean wheel install | clean Windows install | PASS automated; human install OPEN | `docs/v1.1/INSTALL-WINDOWS.md` |
| Installer detects prerequisites, explains cloudflared, runs doctor/First Flight | `install.ps1`, `uninstall.ps1` | release readiness test + PowerShell parse | clean Windows install/uninstall/upgrade | PASS automated; human install OPEN | `docs/v1.1/INSTALL-WINDOWS.md` |
...
| P0 product surfaces remain ahead of P1/P2 | milestone scope control; excluded feature scan | full suite and progress docs | release review | PASS current V1.1 scope | `KNOWN_LIMITATIONS.md` |
...
| Real-node integration, 10k events, focus/selection stability, recovery | T8 real-process harness and bounded SQL paging | T8 Phase A–C and performance tests | none | PASS automated | `tests/tui/test_t8_phase_c_performance.py` |
...
|---|---|---|---|---|---|---|
| 1 | V1 identities, contacts, commands, and queued relay messages survive migration | staging migration + compatibility CLI | migration/CLI regression suites | real upgrade copy / Release owner | PASS automated; real-profile acceptance OPEN | `tests/test_migrations.py` |
...
| 6 | Dashboard under 2s; direct session under 5s; offline state honest and resumes | T8 performance + CLI scale + durable relay/direct retry + production scheduler | Phase B/C + CLI scale + runtime scheduler | normal network timing / Release owner | PASS automated; human timing OPEN | `tests/test_cli_v11_contract.py` |
