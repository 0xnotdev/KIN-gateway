...
6. **Application Shell Mount & Arena Keymap Scoping (`kin/tui/shell.py`, `kin/tui/app.py`, `kin/tui/keymap.py`, `kin/tui/widgets/session_arena.py`)**: Resolved T6 integration gap where `MainCanvas.compose()` fell through to placeholder text for `active_tab_kind == "session"`. Mounted `SessionArenaWidget` in `MainCanvas`, scoped `_get_active_arena_widget()` to session tabs, wired `action_action_compose_message()` to `ComposeMessageModal`, added `scope: Literal["app", "arena"]` to `KeyBindingSpec`, attached `BINDINGS = build_arena_bindings()` to `SessionArenaWidget`, and verified zero binding leakage outside Arena.
