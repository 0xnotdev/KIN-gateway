...
> **Superseded for implementation by `system-design-v1.1.md`.** This retained document records an earlier iteration, including cross-node tools, which are explicitly out of scope for the V1 checkpoint.
...
| Layer | Choice | Why |
|---|---|---|
| Language / API framework | Python 3.12+, FastAPI | Agreed earlier — best-documented ecosystem for agent protocols; async support built in |
...
mailbox: username, encrypted_blob, received_at, expires_at   -- deleted at 7 days or on fetch
```
...
*(queued-relay is a variant state reached when direct delivery to the peer's endpoint fails at send time and the payload is queued at the relay instead — not an inline step in every task's lifecycle.)*

- `submitted`: created, not yet processed.
- `working`: the receiving node is reasoning about it.
- `queued-relay`: direct delivery to the peer's endpoint failed at send time; the payload has been encrypted and queued at the relay, waiting for the recipient to come online. A task can reach this state from an initial ask or from a reply attempt.
...
Body:
{
  ...
  "message_type": "proposal | counter_proposal | question | answer | confirmation | finalize_proposal | finalize_accept | tool_call | tool_response"
}
...
### 4.4a Cross-node tool calls (bounded, read-only)

When a node receives a message with `message_type == "tool_call"`, it represents a request from the peer agent to execute a read-only local tool on their behalf.

**Hard Security Constraints**:
- **Code-Defined Allowlist**: Only tools in a fixed, code-defined allowlist (`REMOTE_ALLOWED_TOOLS`) of side-effect-free, read-only tools may ever be executed via cross-node invocation.
- **Mandatory Human Approval**: Incoming `tool_call` messages transition task status to `input-required` and generate a draft. They are **never** automatically executed by the node or backend. Execution occurs exclusively after the local user reviews and approves the request in the CLI (`kin respond`).
- **Local-Only Tool Exclusion**: Local side-effecting tools (such as `write_file` or `run_command`) are strictly excluded from `REMOTE_ALLOWED_TOOLS`. Any incoming `tool_call` requesting an unallowed tool is blocked and rejected with an error payload.

**Payload Shapes**:
- `ToolCallPayload`: `{ "tool_name": "string", "arguments": { ... }, "justification": "string" }`
- `ToolResponsePayload`: `{ "tool_name": "string", "result": any, "success": bool, "error": "string | null" }`
...
```
POST /directory/register   { username, public_key, x25519_public_key, endpoint }
GET  /directory/lookup/{username}   -> { public_key, x25519_public_key, endpoint }
POST /relay/mailbox/{username}   { sender_username, encrypted_blob }
GET  /relay/inbox   (authenticated via X-Username, X-Timestamp, X-Signature headers)  -> { messages: [ { sender_username, encrypted_blob } ] }
```
